	...
	foreach my $cell_full_name (keys %{$input_db->{"cell"}}) {
		if ($input_db->{"cell"}{$cell_full_name}{"is_hierarchical_cell"}) {
			...
			if (defined $design_db->{$cell_ref_name}) {
				...
				foreach my $pin (keys %{$input_db->{"cell"}{$cell_full_name}{"pin"}}) {
...
sub link {
	...
	if (defined $input_db->{"pin"}) {
        ...
        foreach my $pin (keys %{$input_db->{"pin"}}) {
            # build full_name and change connection
            my $pin_full_name = $prefix . "/" . $pin;
            if ($input_db->{"pin"}{$pin}{"direction"} eq "in") {
                ...
                $input_db->{"pin"}{$pin}{"full_name"} = $pin_full_name;
            }
        }
	}
...
sub add_prefix_to_array_element {
    ...
    if (not $found_element) {
        print "ERROR: Cannot find $ori_val in input array.\n";
        print "ERROR: prefix = $prefix, ori_val = $ori_val.\n";
        print "Dumping input array below:\n";
        print Dumper $array;
    }
}
